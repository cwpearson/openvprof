...
class NvmlPcieThroughputRecord : public Record
{
public:
  NvmlPcieThroughputRecord(
      time_point &start,
      time_point &stop,
      unsigned int devId,
      unsigned long long kb,
      bool tx) : start_(start), stop_(stop), dev_(devId), kbytes_(kb), tx_(tx) {}

  time_point start_;
  time_point stop_;
  unsigned int dev_;
  unsigned long long kbytes_;
  bool tx_;
  nlohmann::json to_json() const override;
};
