...
class CuptiActivityNvlinkRecord : public Record
{
  public:
    CUpti_ActivityNvLink2 nvlink_;
    CuptiActivityNvlinkRecord(CUpti_ActivityNvLink2 *nvlink) : nvlink_(*nvlink)
    {
        std::memcpy(&nvlink_.portDev0, &(nvlink->portDev0), sizeof(nvlink_.portDev0));
        std::memcpy(&nvlink_.portDev1, &(nvlink->portDev1), sizeof(nvlink_.portDev1));
        std::memcpy(&nvlink_.idDev0, &(nvlink->idDev0), sizeof(nvlink_.idDev0));
        std::memcpy(&nvlink_.idDev1, &(nvlink->idDev1), sizeof(nvlink_.idDev1));
    }
    nlohmann::json to_json() const override;
};
...
void
to_json(nlohmann::json &j, const Record &r);
